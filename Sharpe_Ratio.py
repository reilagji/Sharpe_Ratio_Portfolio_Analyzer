...
if len(tickers) >0:
    ...
    st.bar_chart(s)
else: st.text("A simple introduction to the Sharpe Ratio below.\nSuppose each of these two investments both returned 40% over the time period.\nWhich was a better investment? Black is much more painful to hold, but ultimately\mreturned the same amount.Sometimes it beat green but other times\nit severely underperformed.\nThis volatility makes it more painful to hold black than green.\nTherefore, in order to evaluate each investment we employ the Sharpe Ratio, which\n calculates return divivded by volatility experienced. The trailing 20-yr sharpe ratio of S&P is roughly 0.45. Therfore, when building a portfolio of stocks we want to aim for a Sharpe Ratio > 0.45 within our portfolio.")
